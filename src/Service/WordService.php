...
class WordService
{
    ...
    public function getWordByChars(array $chars, $params = [])
    {
        ...
        $wordObjects = $wordRepository->findWithChars($chars, $this->getSubQueries($chars));
        $words       = [];
        /** @var Word $wordObject */
        foreach ($wordObjects as $wordObject) {
            $words[] = $wordObject->getName();
        }
    ...
    private function filterWords(array $words, array $chars)
    {
        ...
        foreach ($words as $word) {
            $wordCharQuantity = $this->getTheNumberOfCharacters($word);
